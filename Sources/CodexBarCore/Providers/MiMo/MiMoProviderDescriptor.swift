...
struct MiMoAPIFetchStrategy: ProviderFetchStrategy {
    ...
    /// API path should hand off to the web/cookie path whenever the API key
    /// is missing or every probed endpoint rejected it — keep the menu live
    /// when the user has both an API key and a cookie session.
