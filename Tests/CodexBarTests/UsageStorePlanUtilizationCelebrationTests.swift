...
extension UsageStorePlanUtilizationTests {
    ...
    @MainActor
    @Test
    func `identity-less Claude reset celebrations require a second low sample`() async {
        let store = Self.makeStore()
        ...
        let before = snapshot(sessionUsed: 30, weeklyUsed: 40, sessionBoundary: firstSessionBoundary, weeklyBoundary: firstWeeklyBoundary, updatedAt: firstDate)
        let apparentReset = snapshot(
            ...
            updatedAt: firstDate.addingTimeInterval(60))
        let recovered = snapshot(sessionUsed: 31, weeklyUsed: 41, sessionBoundary: apparentReset.primary!.resetsAt!, weeklyBoundary: apparentReset.secondary!.resetsAt!, updatedAt: firstDate.addingTimeInterval(120))

        for current in [before, apparentReset, recovered] {
            await store.recordPlanUtilizationHistorySample(provider: .claude, snapshot: current, now: current.updatedAt)
        }
        ...
        let reset = snapshot(
            sessionUsed: 0,
            weeklyUsed: 0,
            sessionBoundary: recovered.primary!.resetsAt!.addingTimeInterval(5 * 60 * 60),
            weeklyBoundary: recovered.secondary!.resetsAt!.addingTimeInterval(7 * 24 * 60 * 60),
            updatedAt: firstDate.addingTimeInterval(180))
        let confirmedReset = snapshot(sessionUsed: 1, weeklyUsed: 1, sessionBoundary: reset.primary!.resetsAt!, weeklyBoundary: reset.secondary!.resetsAt!, updatedAt: firstDate.addingTimeInterval(240))

        await store.recordPlanUtilizationHistorySample(provider: .claude, snapshot: reset, now: reset.updatedAt)
        #expect(sessionRecorder.events.isEmpty)
        #expect(weeklyRecorder.events.isEmpty)
        await store.recordPlanUtilizationHistorySample(provider: .claude, snapshot: confirmedReset, now: confirmedReset.updatedAt)
