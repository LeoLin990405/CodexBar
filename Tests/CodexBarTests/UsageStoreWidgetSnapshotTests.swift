...
struct UsageStoreWidgetSnapshotTests {
    ...
    @Test
    func `widget snapshot preserves usage display preference`() async throws {
        let suite = "UsageStoreWidgetSnapshotTests-usage-display-preference"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        ...
        let store = UsageStore(
            ...
            settings: settings)
        let snapshot = UsageSnapshot(
            primary: RateWindow(usedPercent: 35, windowMinutes: nil, resetsAt: nil, resetDescription: nil),
            secondary: nil,
            tertiary: nil,
            updatedAt: Date(),
            identity: ProviderIdentitySnapshot(
        ...
        store.persistWidgetSnapshot(reason: "usage-display-preference-test")
        await store.widgetSnapshotPersistTask?.value

        let entry = try #require(widgetSnapshots.last?.entries.first { $0.provider == .claude })
        #expect(entry.usageBarsShowUsed == true)
        #expect(entry.usageRows?.first?.percentLeft == 65)
    }
}
